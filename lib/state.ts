...
export const workspaceTools: FunctionCall[] = [
  ...
  ,
  {
    ...
    description: "Runs an allowlisted Google Workspace automation through the backend Apps Script bridge. Use for Docs, Sheets logs, Calendar events, Gmail drafts, deployment reports, and health checks.",
    isEnabled: true,
    scheduling: FunctionResponseScheduling.INTERRUPT,
    parameters: {
      type: "OBJECT",
      properties: {
        action: { type: "STRING", description: "Apps Script action name.", enum: ["ping", "create_doc", "append_sheet_row", "read_sheet_range", "create_calendar_event", "create_gmail_draft", "send_gmail_email", "create_drive_folder", "create_deployment_report", "log_deployment_status", "check_health_url"] },
  ...
  },
  {
    ...
    description: "Creates and saves a structured storyboard for a Google Vids, tutorial, deployment, or training video.",
    isEnabled: true,
    scheduling: FunctionResponseScheduling.INTERRUPT,
    parameters: {
      type: "OBJECT",
      ...
      required: ["title", "purpose", "scenes"]
    }
  },
  {
    name: "create_video_script_document",
    description: "Creates and saves a video script document for Google Vids or app tutorial videos.",
    isEnabled: true,
    scheduling: FunctionResponseScheduling.INTERRUPT,
    parameters: {
      type: "OBJECT",
      properties: {
        title: { type: "STRING" },
        script: { type: "STRING" },
        target_platform: { type: "STRING", enum: ["google_vids", "youtube", "app_tutorial", "whatsapp", "internal_training"] }
      },
      required: ["title", "script", "target_platform"]
    }
  },
  {
    name: "create_deployment_video_plan",
    description: "Creates and saves a deployment walkthrough video plan for Beatrice, Eburon, VPS, or backend deployments.",
    isEnabled: true,
    scheduling: FunctionResponseScheduling.INTERRUPT,
    parameters: {
      type: "OBJECT",
      ...
      required: ["app_name", "environment", "deployment_steps"]
    }
  },
  {
    name: "register_google_video_asset",
    description: "Registers a Google Vids or Drive video file as a known Beatrice project asset.",
    isEnabled: true,
    scheduling: FunctionResponseScheduling.INTERRUPT,
    parameters: {
      type: "OBJECT",
  ...
  },
  {
    ...
    description: "Creates and saves a page-by-page content plan for a Google Site.",
    isEnabled: true,
    scheduling: FunctionResponseScheduling.INTERRUPT,
    parameters: {
      type: "OBJECT",
  ...
  },
  {
    ...
    description: "Creates and saves copy/content for a Beatrice or Eburon deployment portal page.",
    isEnabled: true,
    scheduling: FunctionResponseScheduling.INTERRUPT,
    parameters: {
      type: "OBJECT",
  ...
  },
  {
    ...
    description: "Creates and saves website copy for a restaurant demo using Beatrice as a voice and WhatsApp assistant.",
    isEnabled: true,
    scheduling: FunctionResponseScheduling.INTERRUPT,
    parameters: {
      type: "OBJECT",
      properties: {
        ...
        features: { type: "ARRAY", items: { type: "STRING" } },
        tone: { type: "STRING", enum: ["friendly", "premium", "casual", "modern", "local"] },
...
const toolsets: Record<Template, FunctionCall[]> = {
  ...
  'personal-assistant': [...personalAssistantTools, ...AVAILABLE_TOOLS, ...workspaceTools].filter(
    (tool, index, arr) => tool.name && arr.findIndex(t => t.name === tool.name) === index
  ),
...
export const useUI = create<{
  ...
  setWorkspaceGenerating: (v: boolean) => void;
}>(set => ({
  isSidebarOpen: true,
  toggleSidebar: () => set(state => ({ isSidebarOpen: !state.isSidebarOpen })),
  activeWorkspaceResult: null,
  setActiveWorkspaceResult: (result) => set({ activeWorkspaceResult: result }),
  isWorkspaceGenerating: false,
  setWorkspaceGenerating: (v) => set({ isWorkspaceGenerating: v })
}));
